...
execute_ssh "dokku apps:exists $APP_NAME || dokku app:create $APP_NAME"

# Set hostname
execute_ssh 'echo "ubuntu-s-1vcpu-512mb-10gb-nyc1-01" | sudo tee /etc/hostname > /dev/null'
execute_ssh 'sudo hostname -F /etc/hostname'
...
execute_ssh "dokku docker-options:add ${APP_NAME} build --build-arg S3_SECRET=$S3_SECRET"

# Configure domains and ports
execute_ssh "dokku domains:add ${APP_NAME} cron.mortality.watch"
execute_ssh "dokku ports:set ${APP_NAME} http:80:3012"

# Install plugin
execute_ssh 'dokku plugin:install https://github.com/michaelshobbs/dokku-hostname.git dokku-hostname'

# Set configuration using SSH
execute_ssh "dokku config:set charts --no-restart \
